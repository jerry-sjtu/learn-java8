...
//val factor = Array.fill(10)(random.nextGaussian().toFloat)
//
...
//
//// model.userFactors.map(r => r(0).asInstanceOf[WrappedArray[Float]]).take(5)
////model.userFactors.map(row => row.getAs[Int]("id")).take(5)
